...
describe('GET /api/pdf', () => {
  ...
  it('returns 400 for disallowed host', async () => {
    const f = vi.fn();
    await expect(GET(makeEvent('https://evil.example/foo.pdf', f))).rejects.toMatchObject({ status: 400 });
    expect(f).not.toHaveBeenCalled();
  });

  it('streams the upstream body on success', async () => {
    const body = new Uint8Array([0x25, 0x50, 0x44, 0x46]); // %PDF
    const f = vi.fn(async () => new Response(body, { status: 200, headers: { 'content-type': 'application/pdf' } }));
